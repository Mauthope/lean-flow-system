    ...
    >
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
        ...
        >
          ...
          <p style={{ fontSize: '0.8125rem', color: '#64748b' }}>{action.requesterEmail}</p>
          ...
          <div style={{ marginTop: '0.75rem', paddingTop: '0.75rem', borderTop: '1px dashed #e2e8f0' }}>
            ...
            <p style={{ fontSize: '0.8125rem', color: '#475569', lineHeight: 1.4 }}>{action.description}</p>
          </div>
        </div>

        {/* Decision Toggle */}
        <div>
          <label className="form-label">Decisão do Supervisor:</label>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
            <button
              ...
              style={{
                ...
                color: decision === 'approve' ? '#047857' : '#64748b',
                fontWeight: 700,
                fontSize: '0.875rem',
                cursor: 'pointer',
              }}
            >
              <CheckCircle2 size={18} color={decision === 'approve' ? '#059669' : '#94a3b8'} />
            ...
            <button
              ...
              style={{
                ...
                color: decision === 'reject' ? '#b91c1c' : '#64748b',
                fontWeight: 700,
                fontSize: '0.875rem',
                cursor: 'pointer',
              }}
            >
              <XCircle size={18} color={decision === 'reject' ? '#dc2626' : '#94a3b8'} />
              Recusar / Não Aprovar
            </button>
          </div>
        </div>
        ...
        {decision === 'approve' ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', animation: 'fadeIn 0.2s ease' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
              <div>
                <label className="form-label">Classificação do Desperdício Lean:</label>
                <select
                  className="form-select"
                  value={wasteCategory}
            ...
            {/* Estimated Cost Avoided and Due Date */}
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
              <div>
                <label className="form-label">Estimativa de Custo Evitado (R$):</label>
                <input
                  type="number"
              ...
              <div>
                <label className="form-label">Prazo de Conclusão (Meta):</label>
                <input
                  type="date"
                  className="form-control"
                  value={dueDate}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', animation: 'fadeIn 0.2s ease' }}>
            <div className="form-group" style={{ margin: 0 }}>
              <label className="form-label" style={{ color: '#991b1b' }}>
                Justificativa da Recusa (visível no rastreamento público):
              </label>
              <textarea
                className="form-textarea"
                rows={3}
                placeholder="Informe o motivo técnico ou estratégico para o não acolhimento da demanda..."
        ...
        >
          ...
          >
            {decision === 'approve' ? 'Confirmar Aprovação & Delegar' : 'Confirmar Recusa'}
          </button>
        </div>
      </form>
    </Modal>
