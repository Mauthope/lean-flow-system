...
export default function AgenteKanbanPage() {
  ...
  const academyProgress = useMemo(() => {
    if (!currentUser) return { readCount: 0, total: 8, readPercent: 0, latestExam: undefined };
    const readArticles = dataService.getAgentReadArticles(currentUser.id);
    const latestExam = dataService.getAgentLatestExam(currentUser.id);
    const total = 8;
    const readPercent = total > 0 ? Math.round((readArticles.length / total) * 100) : 0;
    return { readCount: readArticles.length, total, readPercent, latestExam };
  }, [currentUser, dataVersion]);
